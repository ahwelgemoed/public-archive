...
const TIME_TO_WAIT_FOR_SIM_BOOT = 10 * 1000;

const openLastUsedDevice = async ({ spinner }) => {
  try {
...
const getIdOfOpenDevice = async ({ getIdOfApp }) => {
  try {
    getIdOfApp.start();
...
const installMendixAppOnIos = async ({ idOfOpenDevice }) => {
  console.log("installMendixAppOnIos", idOfOpenDevice);
  //   try {
...
const pathToIosApp = () => {
  return path.join(__dirname, "..", "apks", "ios.app");
};
